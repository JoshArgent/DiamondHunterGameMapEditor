...
public class TileMap {
	...
	public void loadMap(String s) {
		...
		try {
			InputStream in = null;
			if(Game.mapFile != null)
				in = new FileInputStream(Game.mapFile);
			else
			...
			// Reads the coordinates of items, if they exist
			// otherwise set the default coordinates
			String axeStr = br.readLine();
			if(axeStr != null && axeStr.contains(","))
				axe = new Coordinate(axeStr);
			else
				axe = new Coordinate(26, 37);
			String boatStr = br.readLine();
			if(boatStr != null && boatStr.contains(","))
				boat = new Coordinate(boatStr);
			else
				boat = new Coordinate(12, 4);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
